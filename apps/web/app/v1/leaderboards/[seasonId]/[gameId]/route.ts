...
export async function GET(
  ...
  try {
    ...
    const sg = await db
      ...
      .where(
        and(
          eq(seasonGames.seasonId, season.id),
          eq(seasonGames.gameId, game.id),
          eq(seasonGames.seedPolicy, 'fixed-course'),
        ),
      )
      .then((r) => r[0]);
    if (!sg) {
    ...
    const viewer = identity.id ? await upsertProfile(db, identity.id, identity.email) : undefined;
