...
test('Archery matches the Node golden score and hash', async ({ page }) => {
  const index = join(here, 'page', 'index.html');
  await page.goto(`file://${index}`);
  const result = (await page.evaluate(() => {
    const run = (window as unknown as { __STICKWORLD_ARCHERY__: () => Promise<unknown> })
      .__STICKWORLD_ARCHERY__;
    if (!run) throw new Error('archery harness did not load');
    return run();
  })) as AttemptResult;
