...
test('Pickaxe matches the Node golden score and hash', async ({ page }) => {
  const index = join(here, 'page', 'index.html');
  await page.goto(`file://${index}`);
  const result = (await page.evaluate(() => {
    const run = (window as unknown as { __STICKWORLD_PICKAXE__: () => Promise<unknown> })
      .__STICKWORLD_PICKAXE__;
    if (!run) throw new Error('pickaxe harness did not load');
    return run();
  })) as AttemptResult;
