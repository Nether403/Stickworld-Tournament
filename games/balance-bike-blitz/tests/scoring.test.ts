...
async function drive(
  ...
  const sim = balanceBikeBlitzGame.createSimulation({
    seed: SAMPLE_SEED,
    rapier,
...
describe('Bike scoring', () => {
  it('scores progress on the sample inputs', async () => {
    const { events, score } = await drive(SAMPLE_INPUTS, SAMPLE_TICKS);
    expect(score).toBeGreaterThan(0);
    expect(events.some((event) => event.type === 'progress' || event.type === 'air' || event.type === 'checkpoint')).toBe(true);
  });
});
