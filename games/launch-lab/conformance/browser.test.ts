...
test('Launch Lab matches the Node golden score and hash', async ({ page }) => {
  const index = join(here, 'page', 'index.html');
  await page.goto(`file://${index}`);
  const result = (await page.evaluate(() => {
    const run = (window as unknown as { __STICKWORLD_LAUNCH_LAB__: () => Promise<unknown> })
      .__STICKWORLD_LAUNCH_LAB__;
    if (!run) throw new Error('launch-lab harness did not load');
    return run();
  })) as AttemptResult;
