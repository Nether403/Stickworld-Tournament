...
test('Demolition Dive matches the Node golden score and hash', async ({ page }) => {
  const index = join(here, 'page', 'index.html');
  await page.goto(`file://${index}`);
  const result = (await page.evaluate(() => {
    const run = (window as unknown as { __STICKWORLD_DEMOLITION__: () => Promise<unknown> })
      .__STICKWORLD_DEMOLITION__;
    if (!run) throw new Error('demolition-dive harness did not load');
    return run();
  })) as AttemptResult;
