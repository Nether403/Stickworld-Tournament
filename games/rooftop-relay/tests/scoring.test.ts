...
async function drive(
  ...
  const sim = rooftopRelayGame.createSimulation({
    seed: SAMPLE_SEED,
    rapier,
...
describe('Rooftop scoring', () => {
  it('uses comboHundredths for flow', () => {
    expect(comboHundredths(1)).toBe(125);
    expect(comboHundredths(4)).toBe(200);
  });

  it('scores progress on the sample run', async () => {
