...
- [ ] 2.1 Create `packages/sim-core` with `@dimforge/rapier2d-compat` pinned to exactly `0.20.0`.
      _(R1.1, R1.2)_
- [ ] 2.2 `rapier.ts`: async init, decode the inlined WASM, compute SHA-256, expose
...
- [ ] 2.3 `units.ts`: `PIXELS_PER_METRE`, gravity constant, SI-unit helpers. No pixel values
      anywhere in simulation code. _(R2.5)_
- [ ] 2.4 `prng.ts`: xorshift128 over 4 × uint32. Reject all-zero state. `nextInt` via rejection
      sampling, never modulo. Test a known seed's first 1,000 outputs. _(R3)_
- [ ] 2.5 `detmath.ts` + `detmath.internal.ts`: implement `sin`, `cos`, `tan`, `atan2`,
...
- [ ] 2.6 `bodies.ts`: stable creation-index registry. All body creation routes through it.
      `ordered()` returns ascending creation index. _(R5.2)_
- [ ] 2.7 `hash.ts`: 64-bit FNV-1a over the f32 bytes of translation, rotation, linvel, angvel
...
- [ ] 2.8 `stepper.ts`: fixed-timestep accumulator with `MAX_FRAME_DELTA` 0.25 s and
      `MAX_TICKS_PER_FRAME` 15. Expose `interpolationAlpha` for presentation only. _(R2.1–R2.4)_
- [ ] 2.9 Frame-pacing invariance test: the same fixture under uniform 60 Hz, jittery 30–144 Hz,
      and one long stall must yield an identical final hash and score. _(R2.6)_
- [ ] 2.10 Build fixture `stress-01` covering all six primitive families — jointed ragdoll with
      ...
      _(R6.1, R6.2)_
- [ ] 2.11 `runner-node.ts` + `pnpm determinism:node`.
- [ ] 2.12 `runner-browser.ts` + a plain static page, driven by Playwright across Chromium,
      Firefox, WebKit, mobile Chromium, mobile WebKit. No bundler machinery — test `sim-core`,
      not a build pipeline. _(R6.3)_
- [ ] 2.13 Divergence matrix reporter: table of runtime × checkpoint, an agreement summary, the
      earliest divergent tick per disagreeing pair, and a human-readable verdict naming the
      indicated branch. _(R6.4, R6.7)_
- [ ] 2.14 Negative control: a fixture variant using `Math.sin` instead of `detmath.sin`,
      asserted in CI to **diverge**. A harness that cannot fail proves nothing. _(R6.6)_
- [ ] 2.15 On agreement, write the hash series to `conformance/golden/` and assert against it in
      CI thereafter. _(R6.5)_
- [ ] 2.16 CI `determinism` and `negative` jobs, matrix uploaded as an artefact, `determinism`
      marked a required check.
- [ ] 2.17 **Write `docs/adr/0001-determinism-fork.md`**: the full matrix, earliest divergent tick
      per pair, chosen branch (A / B1 / B2 / B3 / B4), and the reasoning.
- [ ] 2.18 **Report the result to the user prominently and stop.** If any branch other than A is
...
- [ ] 3.1 Create `packages/replay`. `format.ts` with the 88-byte header layout from design §8.1.
- [ ] 3.2 `varint.ts`: LEB128 unsigned + zigzag signed, with round-trip property tests.
- [ ] 3.3 `crc32.ts` with known-answer tests.
- [ ] 3.4 `encode.ts`: header, delta-encoded tick + action id + zigzag value body, CRC footer,
      then gzip. _(R7.1, R7.3, R7.5, R7.6)_
- [ ] 3.5 `decode.ts`: a parser for hostile input. Size caps checked **before** allocation.
...
- [ ] 3.6 `recorder.ts`: capture-side. Quantises analog inputs to integers per the game's action
...
- [ ] 3.7 `player.ts`: verification-side. Feeds a `Simulation` from a decoded replay, drives
      exactly `totalTicks`, and compares recomputed score and final state hash.
- [ ] 3.8 `score.ts` in `sim-core`: score event stream, integer-only aggregation, `multiplier` as
      an integer scaled by 100. No float accumulation anywhere. _(R8.1, R8.2, R8.3)_
- [ ] 3.9 Score-stream diff: given client and server streams, report the first divergent tick and
      event so rejections are explainable rather than a shrug. _(R8.4, R8.5)_
- [ ] 3.10 `bin/replay-verify.ts` CLI printing score, tick count, and hash match.
- [ ] 3.11 Property test: for randomly generated valid input streams,
      record → encode → decode → replay yields identical hash and identical score. _(R7.9)_
- [ ] 3.12 Size budget test: a 90-second single-action run encodes under 5 KB compressed. _(R7.7)_
- [ ] 3.13 Adversarial corpus: truncated, oversized, bad magic, bad CRC, unknown format version,
...
- [ ] 4.1 `contract.ts`: `GameManifest`, `ActionDescriptor`, `PhysicsBudget`, `AttemptShape`,
      `ScoreEvent`, `Simulation`, `SimulationContext`, `StickworldGame` exactly as design §9.
      _(R9.1, R9.2, R9.3)_
- [ ] 4.2 Implement and document the per-tick order of operations as an enforced contract:
      inputs (ascending action id) → pre-step → `world.step()` → post-step/scoring → tick++.
      _(R9.4)_
- [ ] 4.3 `budget.ts`: runtime budget enforcement, throwing `BudgetExceededError` in dev and test
      builds when a declared limit is exceeded. _(R9.5)_
- [ ] 4.4 Enforce the presentation boundary: `renderState()` output must not be reachable from
      simulation or scoring. Add a test that mutating a render snapshot cannot affect the hash.
      _(R9.6)_
- [ ] 4.5 Build `packages/game-test-chamber`: a minimal `StickworldGame` — score from gates
...
- [ ] 4.6 Cross-runtime score equality: the same module, seed, and input stream produces an
      identical integer score in a browser and in headless Node. _(R10.3)_
- [ ] 4.7 Budget-violation contract test: a deliberately over-budget variant is caught. _(R10.4)_
- [ ] 4.8 Extract the contract test suite into a reusable exported harness so each of the ten
      real games runs the identical checks. Test Chamber is permanent CI infrastructure, not
      scaffolding to delete. _(R10.2)_
- [ ] 4.9 Wire Test Chamber's conformance run into CI alongside `stress-01`.
...
- [ ] Fresh clone builds and tests green; credential exclusion proven by automated test
- [ ] `docs/competitive-spec.md` written and reviewed
- [ ] Divergence matrix produced; fork branch chosen and recorded in ADR-0001
- [ ] Negative control confirms the harness detects injected non-determinism
- [ ] Replay round-trips to identical hash and score; 90-second run under 5 KB
- [ ] Adversarial replay corpus yields typed errors, zero crashes
- [ ] Test Chamber scores identically in browser and headless Node
- [ ] `detmath` lint ban active and demonstrably failing a build when violated
