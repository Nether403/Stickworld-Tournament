...
- [ ] 1.1 Create `packages/db` with Drizzle, `pg`, and Drizzle Kit. Direct URL
      for migrations, pooled URL for app queries. _(R2.1, R2.4)_
- [ ] 1.2 Implement the DDL in design §4: enums, tables (including
...
- [ ] 1.3 Install the `assert_verified_run` trigger (or equivalent) so
      `verified_results` cannot reference an unverified submission. Test: illegal
      insert is refused. _(R1.3)_
- [ ] 1.4 Seed script: season `ci`, game `test-chamber` (`registry_id = 0`),
      pinned `game_versions` matching Spec 1 constants, fixed-course and
      daily-seed `season_games`. _(R9)_
- [ ] 1.5 Migrations apply and roll back on a fresh database. Document the
      rollback command. _(R2.1)_
- [ ] 1.6 CI `schema` job: Neon branch per PR, migrate, test, teardown. Missing
      secrets on a fork skip with an explicit message; this repo requires them.
      _(R2.2, R2.3)_
- [ ] 1.7 Record production autosuspend = off in `docs/adr/0004-spec2-platform-stack.md`
...
- [ ] 2.1 Enable Neon Managed Better Auth on the development branch. Google +
      GitHub (ADR-0004). Do not enable email/password or magic-link. _(R3.1)_
- [ ] 2.2 `apps/web` Next.js App Router with the Neon Auth client and
      `app/api/auth/[...path]/route.ts` proxy as in current Neon Next docs.
- [ ] 2.3 On first session, upsert `profiles` with internal `user_id` and
      `auth_user_id`. Provider identity is not the PK. _(R3.3)_
- [ ] 2.4 `PUT /v1/me/handle` with uniqueness, `HANDLE_PATTERN`, reserved list,
...
- [ ] 2.5 Ranked routes require a claimed handle. Unauthenticated ranked
      attempts return `UNAUTHENTICATED`. _(R3.5)_
- [ ] 2.6 Confirm (and write one line in ADR-0004 if new): Neon Auth's bundled
...
- [ ] 3.1 `POST /v1/games/:gameId/attempts` issues `attempt_id`, 16-byte seed
...
- [ ] 3.2 Single-use `nonce`. Status `issued`. _(R4.3)_
- [ ] 3.3 Rate limits: 10/user/min, 60/user/hour, 30/IP/min. _(R4.4)_
- [ ] 3.4 Rejection paths, internally distinct, externally non-leaky where
      required: expired, consumed, wrong user, wrong version, tampered token,
      inactive season, daily cap, unauthenticated. _(R4.5)_
- [ ] 3.5 Audit event on every issue and every rejection.
...
- [ ] 4.1 `POST /v1/attempts/:attemptId/finish` accepts base64 gzip replay +
      claimed score string. Cheap checks first (design §6). _(R5.1, R5.3)_
- [ ] 4.2 Persist `runs` + `score_submissions` (`pending`) **before** enqueue.
      Mark attempt `submitted` / nonce consumed in the same transaction. _(R5.4)_
- [ ] 4.3 `apps/worker` claims with `FOR UPDATE SKIP LOCKED`, including stale
      locks. _(R5.4, R5.7)_
- [ ] 4.4 Worker: `decodeReplay` → load Test Chamber + pinned Rapier →
      `playReplay` → compare score **and** hash. Reason taxonomy from design §6.
      _(R5.2, R5.5, R5.6)_
- [ ] 4.5 Success path: `verified_results`, conditional `game_bests` upsert
      (strictly better only), dirty flag. _(R1.3, R6.1)_
- [ ] 4.6 Kill-worker test: lock released, job completes exactly once, no
      double `verified_results`. _(R5.7)_
- [ ] 4.7 Inflated claimed score → `SCORE_MISMATCH` + `first_divergent_tick`.
      Tampered inputs → `STATE_HASH_MISMATCH` or score mismatch. Claimed score
      is never written to `game_bests`. _(R5.8)_
- [ ] 4.8 After 5 claims without success: `jobs.state = failed`,
...
- [ ] 5.1 `game_bests` is the eligibility source. `RANK()` over score desc,
      `achieved_at` asc. Shared places. _(R6.1–R6.3)_
- [ ] 5.2 Snapshot `ranking_snapshots` with `asOf`. `GET /v1/leaderboards/...`
      reads the snapshot, never live window functions on the request path. _(R6.4)_
- [ ] 5.3 Drop-and-rebuild test: payloads byte-identical. _(R6.4)_
- [ ] 5.4 Viewer rank row on the payload even when the viewer is not on the
      current page. Keyset pagination (design §8.1). Index
      `(season_game_id, score DESC, achieved_at ASC)`. _(R6.5)_
- [ ] 5.5 Worse scores do not replace a PB; equal scores keep the earlier row.
...
- [ ] 6.1 Implement `placement` / `tail` integer formulas from
...
- [ ] 6.2 Entrant gate: `< 50` → 0 points + `provisional`. _(R7.4)_
- [ ] 6.3 Non-participation is 0. Only `fixed-course` season games. _(R7.5, R8.5)_
- [ ] 6.4 Tiebreakers: wins → top-10 → integer median (design §10.1) → earliest
      total. Table-drive odd `n` and even `n` (lower central value). _(R7.6)_
- [ ] 6.5 Dirty-flag recompute with 30s floor + hourly cron. Every payload has
      `asOf`. _(R7.7)_
- [ ] 6.6 Season close: freeze snapshot, `frozen = true`, subsequent recomputes
      do not mutate it. _(R7.8)_
- [ ] 6.7 Churn test: add tail-only verified results; top-100 points unchanged.
      _(R7.3)_
- [ ] 6.8 Do **not** ship Best-6-of-10. `snapshot_scope` may include `best6`.
      _(R7.9)_
...
- [ ] 7.1 `daily_boards` + UTC rotation cron. _(R8.1, R8.2, R8.4)_
- [ ] 7.2 Per-day cap of 5 ranked attempts per user per game. _(R8.3)_
- [ ] 7.3 Yesterday archived (`archived_at` set). Today's seed is what issuance
      returns for `seedPolicy: 'daily-seed'`.
- [ ] 7.4 Isolation test: daily verified run leaves championship snapshot
...
- [ ] Migrations apply and roll back on a Neon branch; branch-per-PR live
- [ ] Sign-in works; handle claim unique
- [ ] Attempt issuance; every rejection path distinct internally
- [ ] Honest run verifies; inflated claim rejected; tampered inputs rejected
      with a first-divergence location
- [ ] Worker killed mid-job retries without double-crediting
- [ ] Per-game board: order, ties, my-rank, rebuild identical
- [ ] Championship hybrid correct at 100th and 50-entrant gate; top-100 stable
      under tail churn
- [ ] Daily ladder rotates, caps, archives, and cannot touch championship
