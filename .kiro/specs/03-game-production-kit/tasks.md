...
- [ ] 1.1 Workspace: `'games/*'` in `pnpm-workspace.yaml`. Pin `phaser` `4.2.1`
...
- [ ] 1.2 Auth shell: Google + email signup/sign-in; **remove GitHub button**.
      Wrapper in `apps/web/lib/auth/email.ts`. Catalogue cards for Hookline
      (and a disabled/hidden Pickaxe card until Task 3). _(R6)_
- [ ] 1.3 `packages/game-host` with unit tests: countdown does not step;
...
- [ ] 1.9 Playwright: practice completes without auth; ranked without session
...
- [ ] 2.0 **Baseline freeze.** Note the git SHA of Task 1 goldens. Do not
      regenerate them in this task. _(R2.3)_
- [ ] 2.1 `@stickworld/physics-kit`: rope, anchor, raycast attach, gate sensor,
      impact sensor. Lint ban on this package. Hookline simulation imports it.
      _(R2.2)_
- [ ] 2.2 `@stickworld/scoring`: combo/streak hundredths. Hookline uses it.
- [ ] 2.3 `@stickworld/input`: record-on-change. Hookline client uses it.
- [ ] 2.4 `@stickworld/ui`: tokens + countdown + pause + results + leaderboard
      widget + PB toast. Shell and play island use them. _(R5.1)_
- [ ] 2.5 `@stickworld/telemetry`: no-op `emit`. Host may call it. Games run
      without it. _(ADR-0005)_
- [ ] 2.6 Re-run Task 1 determinism + replay fixture. **Byte-identical** to
      2.0. If not, revert the extract — do not update goldens. _(R2.3)_
- [ ] 2.7 Measure `/play/hookline-sprint` client JS+CSS gzip **excluding**
...
- [ ] 3.1 `scripts/check-game-integration.mjs` (or equivalent) fails `verify`
      if a `games/*` package misses design §11 files. Hookline already passes.
      _(R3.1)_
- [ ] 3.2 `games/pickaxe-ascent` simulation from `docs/games/pickaxe-ascent.md`
...
- [ ] 3.3 Pickaxe goldens, scoring tests, contract-suite, replay size ≤ 15 KB
      compressed for the sample (or a 120 s synthetic aim stream). _(R4.2)_
- [ ] 3.4 Pickaxe `./client` + `/play/pickaxe-ascent`. Same host, same UI
      package. Touch + keyboard. _(R5)_
- [ ] 3.5 Seed `registry_id = 2` + `GAMES` map line **only** platform edits.
...
- [ ] 3.6 Playwright lazy-load: Hookline play must not request Pickaxe client
      chunk and vice versa. Integration effort table filled in
      `docs/games/pickaxe-ascent.md`. _(R4.4, R5.2)_
- [ ] 3.7 If any extra platform file changed, write the kit finding and stop
      for review. _(R4.1)_
...
- [ ] Hookline ranked path, no manual SQL
- [ ] Google + email shown; GitHub not shown
- [ ] Extraction byte-identical
- [ ] Bundle baseline + lazy-load
- [ ] Checklist CI
- [ ] Pickaxe via seam only
- [ ] Inspiration ledgers already in this deepening PR; keep them updated if
      names change
